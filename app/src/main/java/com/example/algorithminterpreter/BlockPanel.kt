...
data class PositionedBlock(
    ...
    val zIndex: Float = when (block) {
        is IntVariable, is IntArray, is Assignment -> 2f
...
class If(blockId: UUID) :
    Block(id = blockId, color = Color(0xFFBD3FCB)) {
    var condition : String = ""
    var body = BlocOfBlock(blockId = UUID.randomUUID())
...
class IfElse(id: UUID) :
    Block(id, Color(0xFFFFAD19), heightInDP = 95.dp, widthInDp = 250.dp) {
    var condition: String = ""
    var ifBody = BlocOfBlock(UUID.randomUUID())
    var elseBody = BlocOfBlock(UUID.randomUUID())
    ...
    override fun getHeightInPx(density: Density): Float {
        ...
        inBox += elseBody.getHeightInPx(density)
        inBox += with(density) { (standardHeight).toPx() }
    ...
    override fun getWidthInPx(density: Density): Float {
        ...
        inBox = max(inBox, elseBody.getHeightInPx(density))
        for (block in ifBody.blocks) {
...
class While(id: UUID) :
    Block(id, Color(0xFFFF5755), heightInDP = 350.dp, widthInDp = 255.dp) {
    var condition: String = ""
    ...
    override fun getHeightInPx(density: Density): Float {
        var res = WhileBody.getHeightInPx(density)
...
class ConsoleRead(id: UUID, val text: String = "console.read") :
    Block(id, Color(0xFF9A66FF), heightInDP = 95.dp, widthInDp = 250.dp) {
    var elem: String = ""
}
...
fun BlockPanel(onBlockClick: (Block) -> Unit) {
    ...
    Column(
        ...
        blocks.forEach { block ->
            if (block is IntVariable || block is IntArray || block is Assignment) {
                Box(
                    modifier = Modifier
